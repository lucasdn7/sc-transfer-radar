...
interface ContractTransferStats {
...
export function ContractStatusOverview() {
  const { data: stats, isLoading } = useQuery<ContractTransferStats>({
    queryKey: ['contract-transfer-progress'],
    queryFn: async () => {
      // Buscar processos com contrato assinado
      ...
      // Buscar parcelas pagas dos contratos assinados
      ...
      if (processIds.length > 0) {
        ...
        valorRepassado = parcels?.reduce((sum, p) => sum + (p.value || 0), 0) || 0;
      }
    ...
    <Card className="shadow-lg">
      <CardHeader>
        <CardTitle className="text-lg flex items-center justify-between">
          <span>Status dos Contratos Firmados</span>
